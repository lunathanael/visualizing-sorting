...
template <typename T, typename Rng> T vis::BogoSort<T, Rng>::begin() const {
    return itr1;
}

template <typename T, typename Rng> T vis::BogoSort<T, Rng>::end() const {
    return itr2;
}

template <typename T> T vis::BubbleSort<T>::next() {
    if (std::distance(itr1, itr2) < 1) {
        return itr2;
    }
...
template <typename T, typename Rng> bool vis::BogoSort<T, Rng>::is_done() const {
    return std::is_sorted(itr1, itr2);
}

template <typename T, typename RNG> void vis::BogoSort<T, RNG>::next() {
